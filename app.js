...
async function fetchSSLExpiry(domain) {
  try {
    ...
    var res = await fetch(url, {
      signal: AbortSignal.timeout(8000) /* generous timeout — crt.sh can be slow */
    });
...
async function checkDomain(domain, fullScan) {
   ...
   * ──────────────────────────────────────────────────────────────── */
  if (entry && (entry.sslExpiry === null || entry.sslExpiry === undefined)) {
    /* Fire SSL check in background — don't await, let it update when ready */
    fetchSSLExpiry(domain).then(function(result) {
      if (result && entry) {
        entry.sslExpiry = result.expiry;
        entry.sslIssuer = result.issuer;
        /* Re-render the single row once SSL data arrives */
        renderTable();
        updateStats();
      }
    });
 ...
 * Shows visual feedback if rate-limited.
 */
function triggerRefresh() {
  var now = Date.now();

  /* Anti-spam: show feedback if too soon */
  if (_checkRunning || now - _lastCheckAll < CHECK_ALL_MIN_GAP) {
    var btn = document.getElementById('btn-refresh');
    if (btn) {
      var orig = btn.textContent;
      btn.textContent = _checkRunning ? '⏳ Running…' : '⏳ Wait…';
      btn.disabled = true;
      setTimeout(function() {
        btn.textContent = orig;
        btn.disabled = false;
      }, Math.max(2000, CHECK_ALL_MIN_GAP - (now - _lastCheckAll)));
    }
    return;
  }
...
async function loadDomainList() {
  ...
  /* Initialise empty state for all domains */
...
function checkFirstUse() {
  if (PIN_HASH !== DEFAULT_PIN_HASH) {
    /* User already has a custom PIN — go straight to dashboard */
    initDashboard();
    return;
  }
  /* Default PIN detected — show the set-PIN modal */
...
function spConfirm() {
  ...
  spPersistHash(newHash).then(function(saved) {
    ...
    if (!saved) {
      ...
      alert('PIN set! Your new PIN is active. To make it permanent, open index.html and set PIN_HASH to: ' + newHash);
    }
    initDashboard();
  });
}
